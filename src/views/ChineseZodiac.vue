<template>
  <div class="zodiac-hub">
    <div class="content">
      <div class="badge">🐉 生肖命理</div>
      <h1 class="headline">十二生肖</h1>
      <p class="subhead">中国传统生肖文化，查看你的属相特质</p>

      <div class="sign-grid">
        <div v-for="(s, key) in signs" :key="key" class="sign-card">
          <span class="sign-emoji">{{ s.emoji }}</span>
          <span class="sign-name">{{ s.name }}</span>
          <span class="sign-elem">{{ s.elem }}</span>
        </div>
      </div>
    </div>
    ...
    <nav class="tab-bar">
      <div class="pill">
        ...
        <div class="tab" @click="$router.push('/zodiac')"><span class="tab-icon">★</span><span class="tab-label">星座</span></div>
...
const signs = chineseZodiacData
</script>

<style scoped>
.zodiac-hub { min-height: 100dvh; background: var(--color-bg); display: flex; flex-direction: column; }
.content { flex: 1; padding: 32px 20px 20px; display: flex; flex-direction: column; align-items: center; gap: 28px; }
...
.subhead { font-size: 16px; color: var(--color-text-secondary); text-align: center; }
.sign-grid { width: 100%; display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
.sign-card { padding: 20px 8px; background: var(--color-white); border-radius: var(--radius-lg); display: flex; flex-direction: column; align-items: center; gap: 6px; }
.sign-emoji { font-size: 36px; }
.sign-name { font-family: var(--font-heading); font-size: 14px; color: var(--color-text); }
.sign-elem { font-size: 12px; color: var(--color-text-secondary); }
